	...
	for (let i = 0; i < arr.length; i++) {
	  for (let j = 0; j < arr[i].length; j++) {
		...
		if (n > 0.55) {
		  ...
		  let yoff = height / 2 - offset + j * (offset * 2 / arr.length);
		  ...
		  rect(xoff + nMap * offset * 2 / arr.length, yoff + nMap * offset * 2 / arr.length, offset * 2 / arr.length, offset * 2 / arr.length);
		}
	  }
	}
  }
