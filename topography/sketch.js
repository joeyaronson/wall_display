  ...
  class Tile {
	...
	display() {
	  ...
	  if ((h > 525 && h < 575) || (h > 650 && h < 675)) {
		// emissiveMaterial(colors.trunk);
		emissiveMaterial(10, 100, 13);
		push();
		let treeOff = h / 2 + SIZE / 2;
		translate(0, 0, treeOff);
		box(this.w / 3, this.w / 3, SIZE);
  
		// emissiveMaterial(colors.leaves);
		emissiveMaterial(32, 100, 13);
	  ...
	  if (h < 500) {
		...
		emissiveMaterial(10, 40, 69 - colOff);
	  } else if (h >= 500 && h < 600) {
		colOff = map(h, 500, 600, 0, 30);
  
		emissiveMaterial(40, 51, 57 - colOff);
	  } else if (h >= 600) {
		colOff = map(h, 600, 800, 30, 0);
  
		emissiveMaterial(40, 4, 50 - colOff);
	  }
