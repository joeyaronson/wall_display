...
function draw() {
  ...
  specularMaterial(cOffset+(frameCount / 2) % 100, 50, 100);
...
class Tile {
  ...
  display() {
    push();
    let zHeight =
      noise((this.x + frameCount / 2) * NOISE_VAL, this.y * NOISE_VAL) *
        BOX_SIZE *
        10 -
      BOX_SIZE / 3;
    specularMaterial((zHeight + cOffset + frameCount / 2) % 100, 100, 100);
